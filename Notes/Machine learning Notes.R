...
ggplot(pca_vals, aes(PC1, PC2, color =  iris$Species))+
  geom_point()+
  theme_minimal()
...
##Get rid of NAs
## Only use na.omit when you have specifically selected for 
...
replaceWithMeans <- mutate(starwars, 
                           mass = if_else(is.na(mass),
                                          mean(mass),
                                          mass))

##Encoding categories as factors or integers 
#If categorical variables is a character, make it a factor 
...
##Classification dataset splits (iris)
...
#1. filter data to only 2 groups in categorical variables of interest
# 2. Make the categorical variables a factor
...
boost_class_fit$evaluation_log
...
reg_results$boost_pred <- predict(boost_reg_fit, reg_fit)$.pred
reg_results$forest_pred <- predict(forest_reg_fit)$.pred
...
class_results$lm_pred <- predict(log_fit, reg_test)$.pred
class_results$boost_pred <- predict(boost_reg_fit, reg_fit)$.pred
class_results$forest_pred <- predict(forest_reg_fit)$.pred
